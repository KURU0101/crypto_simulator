...
`real_data_external_signal_series_comparison.example.json` は external signal の consumption series 差分を比較する最小例です。matching、blended 0.7/0.3、blended 0.5/0.5 を同じ real-data returns 上で比較できます。
...
`sns_collector.reddit.example.json` は Reddit の `/r/CryptoCurrency/new.json` を 1 ソースだけ取得し、正規化済み SNS signal と観測 summary を `var/sns_signals/reddit_subreddit_new_json/<run_id>/` に保存する最小構成です。record には `dedup_key` を持たせ、summary には `mention_count_summary` と `duplicate_count` を含めます。実行は `python3 scripts/run_sns_collector.py --config config/sns_collector.reddit.example.json` です。
`sns_collector.youtube.example.json` は YouTube の複数公開 channel RSS を 3 group に束ねて取得し、正規化済み SNS signal と観測 summary を `var/sns_signals/youtube_channel_rss/<run_id>/` に保存する最小構成です。group には `group_theme` と group-level `publisher_type`、channel には `channel_id` / `channel_label` / `publisher_type` / `theme_tags` を持たせます。summary には `group_distribution` / `group_theme_distribution` / `publisher_type_distribution` / `channel_distribution` / `duplicate_count` を含めます。実行は `python3 scripts/run_sns_collector.py --config config/sns_collector.youtube.example.json` です。
`sns_collector.hacker_news.example.json` は Hacker News の公開 API で `topstories` の ID 一覧を取り、`item/{id}` を必要件数だけ取得して正規化済み SNS signal と観測 summary を `var/sns_signals/hacker_news_public_api/<run_id>/` に保存する最小構成です。summary には `score_summary` / `comment_count_summary` / `story_type_distribution` / `duplicate_count` を含めます。実行は `python3 scripts/run_sns_collector.py --config config/sns_collector.hacker_news.example.json` です。
SNS collector の summary は、共通項目をトップレベルに残しつつ、source 固有項目を `source_specific` にも集約します。`mention_count` の意味は source ごとに異なるため、`source_specific.mention_count_semantics` で確認します。
`news_collector.example.json` は CoinDesk RSS、`news_collector.sec.example.json` は SEC Press Releases RSS、`news_collector.federal_reserve.example.json` は Federal Reserve Board All Press Releases RSS を 1 ソースずつ取得し、正規化済みニュース signal と観測 summary を `var/news_signals/<collector_source>/<run_id>/` に保存する最小構成です。正規化後 record には `dedup_key` を持たせ、summary には `category_distribution` と `duplicate_count` を含めます。どれも実行は `python3 scripts/run_news_collector.py --config <config_path>` です。
統合観測導線は専用 config を増やさず、保存済み `var/news_signals/` と `var/sns_signals/` をそのまま読みます。実行は `python3 scripts/observe_external_signals.py`、必要なら `--root-dir` / `--signal-type` / `--source` / `--group-by` / `--latest-only` / `--format` を指定します。
