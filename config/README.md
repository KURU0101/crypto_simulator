...
`news_collector.example.json` は CoinDesk RSS、`news_collector.sec.example.json` は SEC Press Releases RSS を 1 ソースずつ取得し、正規化済みニュース signal と観測 summary を `var/news_signals/<collector_source>/<run_id>/` に保存する最小構成です。正規化後 record には `dedup_key` を持たせ、summary には `category_distribution` を含めます。どちらも実行は `python3 scripts/run_news_collector.py --config <config_path>` です。
